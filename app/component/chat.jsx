...
export default function Chat ({ settings }) {
    ...
    const find_msg = ( id ) => {
        ...
        let el = $(`.chat-div .content-chat #${id}`);
        if ( !el.length ) return;
    ...
    const write_message = (id, content, date, type, link, name, size, sender) => {
        ...
        if ( diff > 0 ) {
            ...
            msg.appendTo(".chat-div .content-chat").fadeIn(300).css("display", "flex").attr("id", id);
            msg.find("p").html(shorten_date(date));
        }

        let cls = '';
        if ( sender === get_session('user') && get_session('user') ) cls = "sender";
        ...
        setTimeout(_ => {
            $(".chat-div .content-chat")[0].scrollBy(0, 10000);
        }, 100);
    ...
    const send_message = async() => {
        ...
        if ( !content ) return;
        let data = {id: 0, user: get_session('user'), content: content, type: "text"};
        ...
        if ( response.user_id ) set_session('user', response.user_id);
    ...
    const active_messages = async() => {
        ...
        setCounter(0);
        await api('active-messages', {user: get_session('user')});
    ...
    const start_chat = () => {
        ...
        if ( $(".chat-div .content-chat #none-active").length ) find_msg("none-active");
        else down();
        active_messages();
        ...
        setTimeout( _ => {
            ...
            if ( !messages.length) {
                ...
                write_message (0, content, "Just now", "text", "", "", "", 'user');
    ...
    const hide_chat = () => {
        ...
        if ( !counter ) $(".chat-div .content-chat").find("#none-active").remove();
        $('body').removeClass("no-scroll");
    
    }
    const down = () => {

        let top = $(".chat-div  .content-chat")[0].scrollHeight + 10000;
        $(".chat-div .content-chat")[0].scrollTo(0, top);
        setTimeout( _ => { $(".chat-div .content-chat")[0].scrollTo(0, top); }, 100);
    ...
    const socket_message = ( data ) => {
        ...
        if ( !check_hidden(".chat-div") ) {
            ...
            else $(".chat-div .content-chat #none-active p").html(`( ${counter+1} ) not readen`);
    ...
    useEffect(() => {
        ...
        let not_readen = messages.filter(item => !item.active && item.sender !== get_session('user')).length;
        let alert_counter = true;
        setCounter(not_readen);

        messages.forEach((item) => {

            if ( !item.active && item.sender !== get_session('user') && alert_counter ) {
    ...
    useEffect(() => {
        ...
        $(".chat-div .content-chat").on("scroll", function(){
